...
def model_prelu(X, w_h, w_h2, w_o, p_use_input, p_use_hidden, a_1, a_2):
    X = dropout(X, p_use_input)
    # first hidden layer, activation function = relu
...
def model_conv(X, w_1, w_2, w_3, w_h2, w_o, p_use_input, p_use_hidden):
...
def simple_nn():
    ...
    noise_h, noise_h2, noise_py_x = model_prelu(X, w_h, w_h2, w_o, 0.8, 0.5, a_1
            , a_2)
    h, h2, py_x = model_prelu(X, w_h, w_h2, w_o, 1., 1., a_1, a_2)
...
def conv_nn():
    ...
    w_h2 = init_weights((625, 12))
    w_o = init_weights((625, 10))

    noise_h, noise_h2, noise_py_x = model_conv(X, w_1, w_2, w_3, w_h2, w_o, 0.8, 0.5)
    h, h2, py_x = model_conv(X, w_1, w_2, w_3, w_h2, w_o, 1., 1.)
    #noise_h, noise_h2, noise_py_x = model(X, w_h, w_h2, w_o, 0.8, 0.5)
    #h, h2, py_x = model(X, w_h, w_h2, w_o, 1., 1.)
    y_x = T.argmax(py_x, axis=1)
